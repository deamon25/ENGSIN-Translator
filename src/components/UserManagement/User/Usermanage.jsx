...
const UserManage = () => {
    ...
    const downloadPDF = () => {
        ...
        // Table headers
        const headers = ['ID', 'Name', 'Email', 'Phone No'];

        // Table data
        const data = users.map(user => [user._id, `${user.firstname} ${user.lastname}`, user.email, user.phone]);
            ...
            <div className="flex mb-10 justify-between">
                ...
                <div className="flex">
                    ...
                    <Link to={'/admindash/users/usercreate'}>
                        <button type="button" className="items-center text-sm font-medium rounded-lg text-white bg-[#63b0c9] hover:bg-[#5296a7] p-2 flex mx-4 px-3">Add New User<span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 font ml-1">
                        ...
                        <table id="user-table" className="min-w-full divide-y divide-gray-200 ">
                            <thead className="bg-gray-200 rounded-lg">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3 text-start text-xs font-bold text-dark uppercase">Phone No</th>
                            ...
                            <tbody className="divide-y divide-gray-200 text-start">
                                {filteredUsers.map((user) => (
                                    <tr key={user._id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{user.phone}</td>
