...
function DetailsDash() {
  ...
  useEffect(() => {
    fetchHandler().then((data) => {
      // Add default 'likes' and 'liked' properties to each item
      const updatedInven = data.inven.map((item) => ({
        ...item,
        likes: item.likes || 0, // Default likes to 0 if undefined
        liked: false,           // Default liked status to false
      }));
  ...
  const handleLike = (index) => {
    const updatedInven = [...inven];
    // Toggle the liked status and update the like count accordingly
    updatedInven[index].liked = !updatedInven[index].liked;
    updatedInven[index].liked
      ? updatedInven[index].likes += 1
      : updatedInven[index].likes -= 1; // Decrease likes if unliked
    setInven(updatedInven);
  };
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "center" }}>
            <thead>
              <tr style={{ backgroundColor: "#588BAE", color: "#fff", borderBottom: "2px solid #ddd" }}>
                ...
                <th style={{ padding: "15px", border: "1px solid #ddd" }}>Likes</th> {/* New column for likes */}
              </tr>
            </thead>
            <tbody>
              {inven.map((item, index) => (
                <tr key={index} style={{ borderBottom: "1px solid #ddd", transition: "background-color 0.3s" }}>
                  ...
                  <td style={{ padding: "15px", backgroundColor: "#f9f9f9" }}>
                    <button
                      onClick={() => handleLike(index)} // Call handleLike function on click
                      style={{
                        backgroundColor: item.liked ? "#28a745" : "#000000", // Change color when liked
                        color: "#fff",
                        padding: "5px 10px",
                        borderRadius: "5px",
                        border: "none",
                    ...
                    >
                      {item.liked ? "Liked" : "Like"} {/* Toggle button label */}
                    </button>
